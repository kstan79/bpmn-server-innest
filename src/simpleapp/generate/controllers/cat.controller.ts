...
// import * as cattype from '../types/cat.type';
import * as types from '../types';
import * as schemas from '../apischemas';
//import * as catapischema from '../apischemas/cat.apischema';
...
const doctype = 'cat'.toUpperCase();
@ApiTags(doctype)
@Controller(doctype.toLowerCase())
export class CategoryController extends SimpleAppAbstractController<
  CategoryService,  
  schemas.Category,
  types.Category> {
  ...
  @Get()
    @Roles(Role.SuperAdmin,Role.SuperUser,Role.Category_search)
    @HttpCode(200)
  @ApiResponse({
    status: 200,
    description: 'Found',
  ...
  @Post()
    @Roles(Role.SuperAdmin,Role.SuperUser,Role.Category_create)
    
  @ApiResponse({
    status: 201,
    description: 'success',
    type: schemas.Category  })
  @ApiResponse({ status: 400, description: 'bad request' })
  @ApiResponse({ status: 500, description: 'internal error' })
  @ApiBody({ description: 'Data',type:schemas.Category })
  @ApiOperation({ operationId: 'runCreate' })
  async create(@AppUser() appuser: UserContext,@Body() data: schemas.Category) {    
...
/***************************** end status control api definitions *****************************************/
  ...
  @ApiResponse({
    ...
    @Roles(Role.SuperAdmin,Role.SuperUser,Role.Category_update)
    
  @ApiResponse({ status: 404, description: 'Document not found' })
  @ApiResponse({ status: 500, description: 'Internal error' })
  @ApiBody({ description: 'Data',type: schemas.Category })
  @ApiOperation({ operationId: 'runUpdate' })
  async update(@AppUser() appuser: UserContext,@Param('id') id: string, @Body() data: schemas.Category) {    
  ...
  async patch(@AppUser() appuser: UserContext,@Param('id') id: string, @Body() data: schemas.Category) {    
    return await this._patch(appuser,id, data) ;
  }

  @Delete(':id')
    @Roles(Role.SuperAdmin,Role.SuperUser,Role.Category_delete)
