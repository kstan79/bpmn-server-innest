...
// import * as orgtype from '../types/org.type';
import * as types from '../types';
import * as schemas from '../apischemas';
//import * as orgapischema from '../apischemas/org.apischema';
...
const doctype = 'org'.toUpperCase();
@ApiTags(doctype)
@Controller(doctype.toLowerCase())
export class OrganizationController extends SimpleAppAbstractController<
  OrganizationService,  
  ...
  @Get()
    @Roles(Role.SuperAdmin,Role.SuperUser,Role.Organization_search)
    @HttpCode(200)
  @ApiResponse({
    status: 200,
    description: 'Found',
  ...
  @Post()
    @Roles(Role.SuperAdmin,Role.SuperUser,Role.Organization_create)
    
  @ApiResponse({
    status: 201,
    description: 'success',
    type: schemas.Organization  })
  @ApiResponse({ status: 400, description: 'bad request' })
  @ApiResponse({ status: 500, description: 'internal error' })
  @ApiBody({ description: 'Data',type:schemas.Organization })
  @ApiOperation({ operationId: 'runCreate' })
  async create(@AppUser() appuser: UserContext,@Body() data: schemas.Organization) {    
...
/***************************** end status control api definitions *****************************************/
  ...
  @ApiResponse({
    ...
    @Roles(Role.SuperAdmin,Role.SuperUser,Role.Organization_update)
    
  @ApiResponse({ status: 404, description: 'Document not found' })
  @ApiResponse({ status: 500, description: 'Internal error' })
  @ApiBody({ description: 'Data',type: schemas.Organization })
  @ApiOperation({ operationId: 'runUpdate' })
  async update(@AppUser() appuser: UserContext,@Param('id') id: string, @Body() data: schemas.Organization) {    
  ...
  async patch(@AppUser() appuser: UserContext,@Param('id') id: string, @Body() data: schemas.Organization) {    
    return await this._patch(appuser,id, data) ;
  }

  @Delete(':id')
    @Roles(Role.SuperAdmin,Role.SuperUser,Role.Organization_delete)
