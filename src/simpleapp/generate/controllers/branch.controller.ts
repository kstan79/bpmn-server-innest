...
// import * as branchtype from '../types/branch.type';
import * as types from '../types';
import * as schemas from '../apischemas';
//import * as branchapischema from '../apischemas/branch.apischema';
import {
  ApiTags,
  ApiBody,
  ApiResponse,
...
const doctype = 'branch'.toUpperCase();
@ApiTags(doctype)
@Controller(doctype.toLowerCase())
export class BranchController extends SimpleAppAbstractController<
  BranchService,
  schemas.Branch,
  types.Branch
> {
  ...
  @Get()
  @Roles(Role.SuperAdmin, Role.SuperUser, Role.Branch_search)
  @HttpCode(200)
  @ApiResponse({
    status: 200,
    description: 'Found',
  ...
  async autoComplete(
    ...
    @Body() data: types.Branch,
  ) {
    return this._autocomplete(appuser, keyword, data);
  }

  @Post()
  @Roles(Role.SuperAdmin, Role.SuperUser, Role.Branch_create)
  @ApiResponse({
    status: 201,
    description: 'success',
    type: schemas.Branch,
  ...
  @ApiResponse({ status: 500, description: 'internal error' })
  @ApiBody({ description: 'Data', type: schemas.Branch })
  @ApiOperation({ operationId: 'runCreate' })
  async create(@AppUser() appuser: UserContext, @Body() data: schemas.Branch) {
  ...
  @Roles(Role.SuperAdmin, Role.SuperUser, Role.Branch_update)
  @ApiResponse({ status: 404, description: 'Document not found' })
  @ApiResponse({ status: 500, description: 'Internal error' })
  @ApiBody({ description: 'Data', type: schemas.Branch })
  ...
  @Delete(':id')
  @Roles(Role.SuperAdmin, Role.SuperUser, Role.Branch_delete)
  @ApiResponse({
    status: 200,
    description: 'success',
    type: schemas.Branch,
