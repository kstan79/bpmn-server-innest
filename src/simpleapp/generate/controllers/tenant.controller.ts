...
// import * as tenanttype from '../types/tenant.type';
import * as types from '../types';
import * as schemas from '../apischemas';
//import * as tenantapischema from '../apischemas/tenant.apischema';
...
const doctype = 'tenant'.toUpperCase();
@ApiTags(doctype)
@Controller(doctype.toLowerCase())
export class TenantController extends SimpleAppAbstractController<
  TenantService,  
  schemas.Tenant,
  types.Tenant> {
  ...
  @Get()
    @Roles(Role.SuperAdmin,Role.Tenant_search)
    @HttpCode(200)
  @ApiResponse({
    status: 200,
    description: 'Found',
  ...
  @Post()
    @Roles(Role.SuperAdmin,Role.Tenant_create)
  ...
  @ApiResponse({ status: 500, description: 'internal error' })
  @ApiBody({ description: 'Data',type:schemas.Tenant })
  @ApiOperation({ operationId: 'runCreate' })
  async create(@AppUser() appuser: UserContext,@Body() data: schemas.Tenant) {    
...
/***************************** end status control api definitions *****************************************/
  ...
  @ApiResponse({
    ...
    @Roles(Role.SuperAdmin,Role.Tenant_update)
    
  @ApiResponse({ status: 404, description: 'Document not found' })
  @ApiResponse({ status: 500, description: 'Internal error' })
  @ApiBody({ description: 'Data',type: schemas.Tenant })
  @ApiOperation({ operationId: 'runUpdate' })
  async update(@AppUser() appuser: UserContext,@Param('id') id: string, @Body() data: schemas.Tenant) {    
  ...
  async patch(@AppUser() appuser: UserContext,@Param('id') id: string, @Body() data: schemas.Tenant) {    
    return await this._patch(appuser,id, data) ;
  }

  @Delete(':id')
    @Roles(Role.SuperAdmin,Role.Tenant_delete)
